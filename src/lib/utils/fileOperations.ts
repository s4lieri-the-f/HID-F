...
export function saveScriptAsTxt(script: DuckyScript, compiledContent: string): void {
  const filename = script.metadata.filename || script.metadata.name;
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!script.metadata.filename) {
          script.metadata.filename = script.metadata.name
            .toLowerCase()
            .replace(/[^a-z0-9\s-]/g, '')
            .replace(/\s+/g, '_')
...
export function createEmptyScript(name: string = 'New Script'): DuckyScript {
  ...
  const filename = name.toLowerCase().replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '_').substring(0, 50) || 'new_script';
