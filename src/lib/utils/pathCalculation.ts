...
function calculatePathOptions(
  ...
  const altSPath = calculateAlternativeSPath(
    from,
    to,
    allNodes,
    excludeNodeIds,
...
function calculateLShapedPath(
  ...
  if (
    !checkPathCollision(from, midPoint, allNodes, excludeNodeIds) &&
  ...
  if (
    !checkPathCollision(from, midPoint2, allNodes, excludeNodeIds) &&
...
function calculateUShapedPath(
  ...
  if (
    !checkPathCollision(from, p1, allNodes, excludeNodeIds) &&
...
function calculateCurvedPath(
  ...
  const curveRadius = Math.min(
    MAX_CURVE_RADIUS,
    Math.max(MIN_CURVE_RADIUS, distance * 0.4),
  );
...
function selectBestPath(
  ...
  const directPaths = pathOptions.filter(
    (path) => path.controlPoints.length === 0,
  );
...
function calculateSPath(
  ...
  const verticalOffset = Math.min(
    maxVerticalOffset,
    Math.max(minVerticalOffset, distance * 0.1),
  );
